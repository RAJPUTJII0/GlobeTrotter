...
export async function getBudget(req, res, next) {
  try {
    const trip = await query('SELECT id, start_date, end_date, budget_limit AS "budgetLimit" FROM trips WHERE id=$1 AND user_id=$2', [req.params.tripId,req.user.id]);
    ...
    return res.json({ breakdown, total, budgetLimit:t.budgetLimit === null ? null : Number(t.budgetLimit), remaining:t.budgetLimit === null ? null : Number(t.budgetLimit)-total, averagePerDay:total/days });
